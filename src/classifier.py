...
def training_test_manual(trainX, testX, trainY, testY, epochs=300, batch=20):
    """
    ...
    """
    model = create_model()
    model.fit(trainX, trainY, epochs=epochs, batch_size=batch, verbose=2)
...
def selector(type_of_learning, fichero, entradas):
    ...
    if type_of_learning == 'auto_neural':
        ...
        if type_of_learning == 'man_neural':
            training_test_manual(trainX, testX, trainY, testY, epochs=300)
...
def main():
    ...
    type_learning = ['auto_neural', 'svm', 'rfc', 'knn']
    for i in range(len(fichero)):
        for j in range(len(type_learning)):
            #selector(type_of_learning=type_learning[j], fichero=fichero[i], entradas=4)
            selector(type_of_learning=type_learning[j], fichero=fichero2[i], entradas=20)
            print(type_learning[j])
