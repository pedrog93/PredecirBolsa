...
def main():
    ...
    dataReal = pd.read_csv('../data/MicrosoftNormal', engine='python', parse_dates=[0], index_col=0,
                            date_parser=transFecha, usecols=[0,11], skipfooter=3)
    adjClose = pd.read_csv('../data/MicrosoftNormalize', usecols=[11], engine='python')#adj close es la columna 11
    ...
    plt.legend(loc='best')
    plt.title('Predicción MSFT a un día')
    plt.show()
